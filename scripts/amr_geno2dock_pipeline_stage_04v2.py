...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        metavar="STR",
        help=(
            "Name of the column in the phenotype file that contains the binary "
            "class labels. If omitted, Stage 04 auto-detects in priority order: "
            "'binary_class' → 'Resistance phenotype'. Use this flag to override "
            "when your column has a different name."
        ),
    )
    parser.add_argument(
        "--resistant-label",
        ...
        metavar="STR",
        help=(
            "String value in the label column that maps to y=1 (resistant / "
            "non-susceptible). If omitted, auto-detected from the column: "
            "'non-susceptible' for binary_class, 'resistant' for Resistance phenotype."
        ),
    )
    parser.add_argument(
        "--susceptible-label",
        ...
        metavar="STR",
        help=(
            "String value in the label column that maps to y=0 (susceptible). "
            "If omitted, auto-detected from the column: "
            "'susceptible' for both binary_class and Resistance phenotype."
        ),
    )
...
def detect_label_column(
    ...
    """
    Determine which column holds the binary phenotype label, and what the
    positive (y=1) and negative (y=0) label strings are.

    Detection priority (when --label-col is not specified):
        1. 'binary_class'        → positive='non-susceptible', negative='susceptible'
        2. 'Resistance phenotype'→ positive='resistant',       negative='susceptible'

    Returns:
        (label_col, positive_label, negative_label)

    The caller may still override positive/negative via --resistant-label /
    --susceptible-label; those overrides are applied after this function returns.
    """
    # Column candidates in priority order, with their default label vocabularies
    CANDIDATES = [
        ("binary_class",        "non-susceptible", "susceptible"),
        ("Resistance phenotype", "resistant",       "susceptible"),
    ]

    if label_col_override is not None:
        if label_col_override not in pheno.columns:
            log.error(
                f"--label-col '{label_col_override}' not found in phenotype file.\n"
                f"Columns present: {list(pheno.columns)}"
            )
            sys.exit(1)
        # Use caller-specified column; default labels depend on which known
        # column was requested, otherwise fall back to resistant/susceptible.
        for col, pos, neg in CANDIDATES:
            if label_col_override == col:
                log.info(f"Label column (explicit): '{label_col_override}' "
                         f"[defaults: pos='{pos}', neg='{neg}']")
                return label_col_override, pos, neg
        # Unknown column name — use generic defaults; user should supply
        # --resistant-label / --susceptible-label explicitly.
        log.info(
            f"Label column (explicit): '{label_col_override}' "
            f"[unknown defaults — expecting --resistant-label / --susceptible-label]"
        )
        return label_col_override, "resistant", "susceptible"

    # Auto-detect
    for col, pos, neg in CANDIDATES:
        if col in pheno.columns:
            log.info(
                f"Label column (auto-detected): '{col}' "
                f"[pos='{pos}', neg='{neg}']. "
                f"Override with --label-col if needed."
            )
            return col, pos, neg

    # Neither column found
    log.error(
        "Could not find a recognised label column in the phenotype file.\n"
        "Expected one of: 'binary_class', 'Resistance phenotype'.\n"
        f"Columns present: {list(pheno.columns)}\n"
        "Use --label-col to specify the correct column name."
    )
    sys.exit(1)
...
def load_phenotypes(
    path: Path,
    label_col_override: str | None,
    resistant_label: str | None,
    susceptible_label: str | None,
) -> pd.DataFrame:
    """
    Load the Stage 01 model_data export and resolve binary labels to y=0/1.

    Supports two phenotype column conventions used across species runs:

        Column 'Resistance phenotype'  (original A. baumannii / single-antibiotic)
            Typical values: 'resistant' | 'susceptible'

        Column 'binary_class'          (multi-antibiotic merge output)
            Typical values: 'non-susceptible' | 'susceptible'

    Auto-detection logic (overridden by --label-col):
        'binary_class' takes priority if present → pos='non-susceptible'
        Falls back to 'Resistance phenotype'     → pos='resistant'

    Both --resistant-label and --susceptible-label override the auto-detected
    defaults when the label vocabulary differs from the above.

    Maps resolved labels to binary integers:
        positive (resistant / non-susceptible) → 1
        negative (susceptible)                 → 0
    ...
    if "Assembly_Accession" not in pheno.columns:
        log.error(
            f"Phenotype file missing required column 'Assembly_Accession'.\n"
            f"Columns found: {list(pheno.columns)}"
        )
        sys.exit(1)

    # ── Detect label column & default vocabularies ────────────────────────────
    label_col, default_pos, default_neg = detect_label_column(pheno, label_col_override)

    # Apply CLI overrides if provided
    pos_label = resistant_label   if resistant_label   is not None else default_pos
    neg_label = susceptible_label if susceptible_label is not None else default_neg

    log.info(
        f"Label mapping: '{pos_label}' → 1  |  '{neg_label}' → 0  "
        f"(column: '{label_col}')"
    )

    pheno = pheno[["Assembly_Accession", label_col]].copy()
    pheno = pheno.drop_duplicates(subset="Assembly_Accession")

    label_map = {pos_label: 1, neg_label: 0}
    pheno["y"] = pheno[label_col].map(label_map)

    unmapped = pheno["y"].isna().sum()
    if unmapped > 0:
        bad_labels = pheno[pheno["y"].isna()][label_col].unique()
        log.warning(
            f"{unmapped} accession(s) have unrecognised values in '{label_col}' "
            f"and will be excluded: {bad_labels}.\n"
            f"  Expected: '{pos_label}' or '{neg_label}'.\n"
            f"  Override with --resistant-label / --susceptible-label if needed."
        )
    ...
    log.info(
        f"Phenotype labels: {n_r} positive (y=1) | {n_s} negative (y=0) "
...
def main() -> None:
    ...
    pheno = load_phenotypes(
        ...
        resistant_label=args.resistant_label,
        susceptible_label=args.susceptible_label,
    )
...
# =============================================================================
#
#  Standard run — 'Resistance phenotype' column, resistant/susceptible labels
#  (A. baumannii × tobramycin, original single-antibiotic format):
#    python amr_geno2dock_pipeline_stage_04.py \
#        --amrfinder-dir amrfinder \
#        --phenotypes model_data.csv \
#        --out-dir ml_data
#
#  Auto-detect 'binary_class' column — non-susceptible/susceptible labels
#  (multi-antibiotic merge output; no flags needed, auto-detected):
#    python amr_geno2dock_pipeline_stage_04.py \
#        --amrfinder-dir amrfinder \
#        --phenotypes model_data_multiab.csv \
#        --out-dir ml_data
#
#  Explicit label column override (custom column name):
#    python amr_geno2dock_pipeline_stage_04.py \
#        --amrfinder-dir amrfinder \
#        --phenotypes model_data.csv \
#        --label-col "AST_class" \
...
#        --keep-unmatched
#
# LABEL COLUMN AUTO-DETECTION LOGIC:
#   Priority 1: 'binary_class'         → pos='non-susceptible', neg='susceptible'
#   Priority 2: 'Resistance phenotype' → pos='resistant',       neg='susceptible'
#   Fallback:   use --label-col + --resistant-label + --susceptible-label
#
# =============================================================================
